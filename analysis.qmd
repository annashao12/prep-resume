...
resume <- read_csv("data/resume.csv", show_col_types = F)
resume$race <- as.factor(resume$race)
# resume |> count(call)
```

```{r}
#| label: model_creation
#| cache: true

fit_bern <- brm(formula = `call` ~ `race`,
                data = resume,
                family = bernoulli(),
                refresh = 0,
                silent = 2, # turn off certain warnings
                seed = 12)
# summary(fit_bern)
# pp_check(fit_bern)
```
...
fit_bern |> 
  ...
  ggplot(aes(x = .epred, fill = race)) + 
    ...
    labs(x = "Probability of Callback",
         y = NULL,
